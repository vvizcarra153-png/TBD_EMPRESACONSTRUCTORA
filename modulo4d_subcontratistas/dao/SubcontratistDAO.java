...
public class SubcontratistDAO {
    ...
    public void crear(Subcontratista subcontratista) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.executeUpdate();
            System.out.println("✓ Subcontratista registrado");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    
    public Subcontratista obtener(int id) {
        String sql = "SELECT * FROM subcontratistas WHERE id_subcontratista = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public List<Subcontratista> obtenerPorEspecialidad(String especialidad) {
        List<Subcontratista> subcontratistas = new ArrayList<>();
        String sql = "SELECT * FROM subcontratistas WHERE especialidad = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public void actualizar(Subcontratista subcontratista) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.executeUpdate();
            System.out.println("✓ Subcontratista actualizado");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void eliminar(int id) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setInt(1, id);
            stmt.executeUpdate();
            System.out.println("✓ Subcontratista eliminado");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
